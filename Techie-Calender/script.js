document.addEventListener('DOMContentLoaded', function() {
    ...
    const renderCalendar = () => {
        ...
        // -- FIX START: Get today's local date components as numbers for a reliable comparison --
        ...
        // Add blank cards for days before the 1st
        ...
        for (let day = 1; day <= lastDateOfMonth; day++) {
            // We use this UTC-based string ONLY for a consistent localStorage key
            const dateForKey = new Date(Date.UTC(year, month, day));
            const dateString = dateForKey.toISOString().split('T')[0];
            
            // -- FIX START: Compare the numbers directly, ignoring timezones completely --
            ...
            card.dataset.date = dateString; // Use the consistent key for the dataset
            ...
            card.innerHTML = `
                ...
                <!-- Ticket/Session Section -->
                <div class="card-section">
                    <label>Total Sessions: <strong class="total-sessions-display">${totalSessions}</strong></label>
                    <div class="ticket-inputs">
                        <div class="flex-fill">
                            <label>Pending</label>
                            <input type="number" min="0" class="form-control form-control-sm pending-input" value="${dayData.pending}">
                        </div>
                        <div class="flex-fill">
                            <label>Resolved</label>
                            <input type="number" min="0" class="form-control form-control-sm resolved-input" value="${dayData.resolved}">
                        </div>
                    </div>
                </div>
                ...
                <div class="card-section mt-2">
                    ...
                    <div class="call-counter">
                        <button class="btn btn-sm btn-outline-danger minus-call-btn" title="Decrease Calls"><i class="fas fa-minus"></i></button>
                        <span class="call-count">${dayData.calls}</span>
                        <button class="btn btn-sm btn-outline-primary plus-call-btn" title="Increase Calls"><i class="fas fa-plus"></i></button>
                    </div>
                </div>
            `;
    ...
    calendarBody.addEventListener('click', (e) => {
        ...
        if (!calendarData[dateString]) {
            calendarData[dateString] = { pending: 0, resolved: 0, calls: 0 };
        }
